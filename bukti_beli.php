...
<?php 
	...
	echo "TANGGAL  : ".$rowhjual['tanggal'];
	echo "NAMA     : ".$rowhjual['namacust'];
	$sqldjual = "select barang.nama, djual.harga, djual.qty, (djual.harga * djual.qty) as jumlah from djual inner join
		barang on djual.idbarang=barang.idbarang where djual.idhjual=$idhjual";
	...
	while($rowdjual = mysqli_fetch_assoc($hasildjual)){
		...
		echo " <td> ".$rowdjual['harga']." </td>";
		echo " <td> ".$rowdjual['jumlah']." </td>";
		echo "</tr>";
